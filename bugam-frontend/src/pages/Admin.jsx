        ...
        {editItem && (
          <div className="modal-overlay" onClick={() => setEditItem(null)}>
            <div className="card modal-content" onClick={e => e.stopPropagation()}>
              ...
              {editItem.type === 'reservacion' && (<form onSubmit={e => { e.preventDefault(); saveReservacion(editItem); setEditItem(null) }}><div className="form-group"><label className="label">Cliente</label><input className="input" value={editItem.cliente_nombre || ''} onChange={e => setEditItem({...editItem, cliente_nombre: e.target.value})} required /></div><div className="form-group"><label className="label">Teléfono</label><input className="input" value={editItem.telefono || ''} onChange={e => setEditItem({...editItem, telefono: e.target.value})} /></div><div className="form-group"><label className="label">Ubicación</label><select className="input" value={editItem.ubicacion_id || ''} onChange={e => setEditItem({...editItem, ubicacion_id: parseInt(e.target.value)})}><option value="">Seleccionar</option>{ubicaciones.map(u => <option key={u.id} value={u.id}>{u.nombre}</option>)}</select></div><div className="form-group"><label className="label">Fecha</label><input type="date" className="input" value={editItem.fecha_reserva || ''} onChange={e => setEditItem({...editItem, fecha_reserva: e.target.value})} required /></div><div className="form-group"><label className="label">Hora</label><input type="time" className="input" value={editItem.hora_reserva || ''} onChange={e => setEditItem({...editItem, hora_reserva: e.target.value})} required /></div><div className="form-group"><label className="label">Personas</label><input type="number" className="input" value={editItem.num_personas || 1} onChange={e => setEditItem({...editItem, num_personas: parseInt(e.target.value)})} /></div><div className="form-group"><label className="label">Notas</label><textarea className="input" value={editItem.notas || ''} onChange={e => setEditItem({...editItem, notas: e.target.value})} /></div>{editItem.id && <div className="form-group"><label className="label">Estado</label><select className="input" value={editItem.estado || 'CONFIRMADA'} onChange={e => setEditItem({...editItem, estado: e.target.value})}><option value="CONFIRMADA">Confirmada</option><option value="COMPLETADA">Completada</option><option value="CANCELADA">Cancelada</option></select></div>}<div className="flex gap-2"><button type="submit" className="btn btn-primary">Guardar</button><button type="button" className="btn btn-secondary" onClick={() => setEditItem(null)}>Cancelar</button></div></form>)}
            </div>
          </div>
        )}
      </div>
