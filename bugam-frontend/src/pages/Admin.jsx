      ...
      <div className="main-content">
        ...
        {loading ? <div className="text-center">Cargando...</div> : (
          <>
            ...
            {activeTab === 'inventario' && (<div><div className="flex justify-between mb-4"><h2>Inventario</h2><button className="btn btn-primary" onClick={() => setEditItem({ type: 'producto' })}>+ Nuevo</button></div><div className="table-responsive"><table className="table"><thead><tr><th>Producto</th><th>Categoría</th><th>Stock</th><th>Stock Mín</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{inventario.map(p => (<tr key={p.id}><td>{p.nombre}</td><td>{p.categoria_nombre}</td><td>{p.stock}</td><td>{p.stock_minimo}</td><td><span className={`badge ${p.stock <= p.stock_minimo ? 'badge-danger' : 'badge-success'}`}>{p.stock <= p.stock_minimo ? 'Bajo' : 'OK'}</span></td><td><button className="btn btn-primary btn-sm" onClick={() => setEditItem({...p, type: 'producto'})}>Editar</button></td></tr>))}</tbody></table></div></div>)}

            {activeTab === 'usuarios' && (<div><div className="flex justify-between mb-4"><h2>Usuarios</h2><button className="btn btn-primary" onClick={() => setEditItem({ type: 'usuario' })}>+ Nuevo</button></div><div className="table-responsive"><table className="table"><thead><tr><th>Usuario</th><th>Nombre</th><th>Rol</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{usuarios.map(u => (<tr key={u.id}><td>{u.username}</td><td>{u.nombre}</td><td>{u.rol_nombre}</td><td><span className={`badge ${u.activo ? 'badge-success' : 'badge-danger'}`}>{u.activo ? 'Activo' : 'Inactivo'}</span></td><td><button className="btn btn-primary btn-sm" onClick={() => setEditItem({...u, type: 'usuario'})}>Editar</button></td></tr>))}</tbody></table></div></div>)}
